...
class Aggregation():
    ...
    def prepare_auxiliary_data(self, args=None, dataset_name=None, num_samples=100, val_dataset=None):
        """
        Prepare auxiliary data loader for PLR analysis or Fine-tuning.
        Samples a subset from the validation/test dataset.
        """
        if args is None:
            args = self.args
            
        # If val_dataset is not provided, try to load it
        if val_dataset is None:
            logging.info(f"Loading dataset {dataset_name} for auxiliary data...")
            _, val_dataset = utils.get_datasets(dataset_name)
            
        if val_dataset is None:
            logging.error("Failed to obtain validation dataset for auxiliary data.")
            return None
        ...
        aux_dataset = Subset(val_dataset, indices)
    ...
    def aggregate_updates(self, global_model, agent_updates_dict, auxiliary_data_loader, current_round=None, auxiliary_data_loader_finetune=None):
        ...
        if self.args.aggr == 'avg':
            ...
            aggregated_updates = self.agg_origin_alignins_clustering_prune_finetune(agent_updates_dict, cur_global_params, global_model, auxiliary_data_loader, current_round=current_round, auxiliary_data_loader_finetune=auxiliary_data_loader_finetune)
            
        vector_to_model(cur_global_params + aggregated_updates * self.server_lr, global_model)
        return aggregated_updates, None
    ...
    def agg_origin_alignins_clustering_prune_finetune(self, agent_updates_dict, flat_global_model, global_model, auxiliary_data_loader, current_round=None, auxiliary_data_loader_finetune=None):
