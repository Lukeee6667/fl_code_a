...
class Agent():
    def __init__(self, id, args, train_dataset=None, data_idxs=None, mask=None, backdoor_train_dataset=None):
        ...
        if self.args.data != "tinyimagenet":
            ...
            data_poison_attacks = {"badnet", "DBA", "neurotoxin", "pgd"}
            if self.id < args.num_corrupt and self.args.attack in data_poison_attacks and self.args.data != 'sen140':
            ...
            elif self.id < args.num_corrupt and self.args.attack in data_poison_attacks and self.args.data == 'sen140':
                self.clean_backup_dataset = copy.deepcopy(train_dataset)
                self.data_idxs = data_idxs
            ...
            runtime_poison = self.args.attack in {"badnet", "DBA", "neurotoxin", "pgd"}
            self.train_dataset = utils.DatasetSplit(train_dataset, data_idxs, runtime_poison=runtime_poison, args=args,
                                                        client_id=id)
        # get dataloader
    ...
    def local_train(self, global_model, criterion, round=None, neurotoxin_mask=None, bypass_a4fl=False):
        ...
        for local_epoch in range(self.args.local_ep):
            ...
            for i, (inputs, labels) in enumerate(self.train_loader):
                ...
                if self.args.attack == "neurotoxin" and neurotoxin_mask is not None and len(neurotoxin_mask) and self.id < self.args.num_corrupt:
                    for name, param in global_model.named_parameters():
                        param.grad.data = neurotoxin_mask[name].to(self.args.device) * param.grad.data
                if self.args.attack == "r_neurotoxin" and neurotoxin_mask is not None and len(neurotoxin_mask) and self.id < self.args.num_corrupt:
                    for name, param in global_model.named_parameters():
            ...
            logging.info("local epoch %d \t client: %d \t mal: %s \t loss: %.8f \t time: %.2f" % (local_epoch, self.id, str(self.is_malicious),
                                                                     minibatch_loss, train_time))
