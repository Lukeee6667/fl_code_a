...
if __name__ == "__main__":
    ...
    # ==========================================
    # IMS Prune + Finetune Logic
    # ==========================================
    if args.aggr == "ims_prune_finetune":
        logging.info("IMS Prune + Finetune: Starting post-training phase...")
        
        # 1. Identify Masks using IMS (Re-run to be sure)
        if auxiliary_data_loader is None:
             # Should have been created earlier, but if not, we are in trouble.
             # Assuming it exists.
             logging.warning("IMS Prune: Auxiliary data loader is None! Using validation set.")
             # Fallback logic if needed, but we added ims_prune_finetune to the check list.
             
        from agg_ims_fast import IMSAggregator
        ims_agg = IMSAggregator(args, args.device)
        
        # Prepare candidate model (global_model)
        candidate_model = copy.deepcopy(global_model)
        candidate_model.eval()
        for param in candidate_model.parameters():
            param.requires_grad = False
            
        prunable_layers = ims_agg._get_prunable_layers(candidate_model)
        
        logging.info("IMS Prune: Identifying structural masks...")
        # Run IMS search
        A_init, S_init = ims_agg.mask_initialization(auxiliary_data_loader, candidate_model, prunable_layers)
        delta_dict = ims_agg.inner_subproblem(auxiliary_data_loader, candidate_model, A_init, S_init, prunable_layers)
        A_final, S_final = ims_agg.outer_subproblem(auxiliary_data_loader, candidate_model, delta_dict, A_init, S_init, prunable_layers)
        
        a_prime_list, _ = ims_agg.compute_mask_and_inverse(A_final, S_final, ims_agg.k)
        
        # Build Global Mask Vector
        mask_dict = {}
        total_pruned = 0
        total_params_count = 0
        
        # Helper to map a_prime back to layer names
        layer_mask_map = {l['name']: m for l, m in zip(prunable_layers, a_prime_list)}
        
        for name, param in global_model.named_parameters():
            parent_name = ".".join(name.split(".")[:-1])
            param_type = name.split(".")[-1] # "weight" or "bias"
            
            if parent_name in layer_mask_map and param_type == "weight":
                # It is a prunable weight
                soft_mask = layer_mask_map[parent_name].detach()
                # Threshold (0.05)
                binary_mask = (soft_mask > 0.05).float()
                
                if binary_mask.numel() != param.numel():
                     # Handle broadcasting for Conv layers: (C_out) -> (C_out, 1, 1, 1)
                     view_shape = [1] * len(param.shape)
                     # Assuming mask corresponds to the first dimension (out_channels)
                     if len(param.shape) > 0:
                         view_shape[0] = -1
                         binary_mask = binary_mask.view(*view_shape).expand_as(param)
                else:
                     binary_mask = binary_mask.view(param.shape)

                mask_dict[name] = binary_mask
                
                # Apply pruning immediately
                param.data *= binary_mask
                
                total_pruned += (1 - binary_mask).sum().item()
            else:
                # Keep
        ...
        logging.info(f"IMS Prune: Pruned {int(total_pruned)}/{total_params_count} parameters.")
        
        # Flatten mask_dict to vector in correct order matching Agent's state_dict iteration
        mask_list = []
        state_dict = global_model.state_dict()
        for name in state_dict:
             if name in mask_dict:
                 mask_list.append(mask_dict[name].contiguous())
             else:
                 mask_list.append(torch.ones_like(state_dict[name]))
        flat_mask = parameters_to_vector(mask_list)
        
        # 2. Lightweight Fine-tuning
        if args.not_finetune_rounds <= 0:
             args.not_finetune_rounds = 5
             
        logging.info("IMS Finetune: Starting fine-tuning for %d rounds..." % args.not_finetune_rounds)
        
        # Use full clean dataset for fine-tuning
        clean_train_dataset, _ = utils.get_datasets(args.data)
        full_train_loader = DataLoader(
            clean_train_dataset, batch_size=args.bs, shuffle=True, 
            num_workers=args.num_workers, drop_last=True
        )
        
        dummy_agent = Agent(
            id=args.num_agents, args=args, train_dataset=clean_train_dataset,
            data_idxs=list(range(len(clean_train_dataset))), backdoor_train_dataset=None
        )
        ...
        for ft_rnd in range(1, args.not_finetune_rounds + 1):
            logging.info("IMS Finetune: Round %d..." % ft_rnd)
            
            # Local Train
            update = dummy_agent.local_train(global_model, criterion, ft_rnd)
            
            # Apply Mask to Update (Constraint: prohibit activation of pruned components)
            # If weight was 0 and update tries to change it, masked_update will be 0.
            # So weight stays 0.
            masked_update = update * flat_mask.to(update.device)
            
            # Apply update
            cur_params = parameters_to_vector([global_model.state_dict()[name] for name in global_model.state_dict()])
            lr_vec = torch.tensor([args.server_lr] * len(cur_params), device=args.device)
            new_params = (cur_params + lr_vec * masked_update).float()
            utils.vector_to_model(new_params, global_model)
        ...
        val_acc = utils.get_loss_n_accuracy(global_model, criterion, val_loader, args, args.rounds + args.not_finetune_rounds, args.num_target)
        asr = utils.get_loss_n_accuracy(global_model, criterion, poisoned_val_loader, args, args.rounds + args.not_finetune_rounds, num_classes=args.num_target)
        poison_acc = utils.get_loss_n_accuracy(global_model, criterion, poisoned_val_only_x_loader, args, args.rounds + args.not_finetune_rounds, args.num_target)
        
        logging.info("Post-Prune+FT Clean ACC:            %.4f" % val_acc)
        logging.info("Post-Prune+FT Attack Success Ratio: %.4f" % asr)
        logging.info("Post-Prune+FT Backdoor ACC:         %.4f" % poison_acc)
