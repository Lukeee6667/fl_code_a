...
class Aggregation():
    ...
    def aggregate_updates(self, global_model, agent_updates_dict, auxiliary_data_loader=None, current_round=None, auxiliary_data_loader_finetune=None):
        ...
        if self.args.aggr=='avg' or self.args.aggr == 'rlr' or self.args.aggr == 'lockdown':          
            ...
            aggregated_updates = self.agg_alignins(agent_updates_dict, cur_global_params, current_round=current_round)
        ...
        elif self.args.aggr == 'ims_prune_finetune':
            # IMS Prune + Finetune (Aggregation phase is same as IMS Fast)
            data_loader = auxiliary_data_loader if auxiliary_data_loader is not None else self.auxiliary_data_loader
            aggregated_updates = self.agg_ims_prune_finetune(agent_updates_dict, cur_global_params, global_model, data_loader, current_round=current_round, auxiliary_data_loader_finetune=auxiliary_data_loader_finetune)
        ...
        elif self.args.aggr == 'alignins_ims_standard':
            # AlignIns + Standard IMS (Config 17)
            alignins_update = self.agg_alignins(agent_updates_dict, cur_global_params)
            data_loader = auxiliary_data_loader if auxiliary_data_loader is not None else self.auxiliary_data_loader
            aggregated_updates = self.agg_ims(agent_updates_dict, cur_global_params, global_model, data_loader, current_round=current_round, initial_update=alignins_update)
    ...
    def agg_ims_prune_finetune(self, agent_updates_dict, flat_global_model, global_model, auxiliary_data_loader, current_round=None, initial_update=None, auxiliary_data_loader_finetune=None):
        """
        IMS Prune + Finetune: Structured Pruning + Lightweight Finetuning
        """
        try:
            from agg_ims_prune import agg_ims_prune_finetune
        except ImportError:
            from src.agg_ims_prune import agg_ims_prune_finetune
        
        if auxiliary_data_loader is None:
            import logging
            logging.warning("IMS Prune: No auxiliary data loader provided! Falling back to FedAvg.")
            if initial_update is not None:
                return initial_update
            return self.agg_avg(agent_updates_dict)
            
        aggregated_update = agg_ims_prune_finetune(
            agent_updates_dict,
            flat_global_model,
            global_model,
            self.args,
    ...
    def agg_alignins(self, agent_updates_dict, flat_global_model, current_round=None):
        """
        ...
        # Logging results
        logging.info(f"--- Round {current_round} AlignIns Detection Results ---")
        ...
        # Log detailed metrics for plotting
        if current_round is not None:
            logging.info(f"Round {current_round} TDA: {[round(x, 4) for x in detection_results.get('tda_scores', [])]}")
            logging.info(f"Round {current_round} MPSA: {[round(x, 4) for x in detection_results.get('mpsa_scores', [])]}")
            logging.info(f"Round {current_round} Grad Norm: {[round(x, 4) for x in detection_results.get('grad_norm_scores', [])]}")
            logging.info(f"Round {current_round} Mean Cos: {[round(x, 4) for x in detection_results.get('mean_cos_scores', [])]}")
            
            logging.info(f"Round {current_round} MZ-score of TDA: {[round(x, 4) for x in detection_results.get('mz_tda', [])]}")
            logging.info(f"Round {current_round} MZ-score of MPSA: {[round(x, 4) for x in detection_results.get('mz_mpsa', [])]}")
            logging.info(f"Round {current_round} MZ-score of Grad Norm: {[round(x, 4) for x in detection_results.get('mz_grad_norm', [])]}")
            logging.info(f"Round {current_round} MZ-score of Mean Cos: {[round(x, 4) for x in detection_results.get('mz_mean_cos', [])]}")
