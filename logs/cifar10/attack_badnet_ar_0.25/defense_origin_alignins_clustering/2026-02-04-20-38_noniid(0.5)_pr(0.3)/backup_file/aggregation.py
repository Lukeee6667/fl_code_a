...
class Aggregation():
    ...
    def prepare_auxiliary_data(self):
        # 简化处理：从测试集中随机采样少量数据作为辅助验证集
        # 注意：这里需要从全局获取数据，暂时假设可以在外部传入或重新加载
        # 由于Aggregation类初始化时不直接持有数据集，这里需要一种机制获取
        # 临时方案：尝试从args或其他地方获取，如果不行则在调用时传入
        # 为了不破坏现有结构，我们假设utils里有获取数据的函数，或者暂不实现具体加载
        pass

    def aggregate_updates(self, agent_updates_dict, current_round=None):
        cur_global_params = parameters_to_vector([ agent_updates_dict[0][name] for name in agent_updates_dict[0] ]).detach()
